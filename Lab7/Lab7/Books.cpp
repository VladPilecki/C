...
{
   struct Book* temp = lst->head;
    struct Book* pNext = NULL;
    while (temp) {
        pNext = temp->next;
        free(temp);
        temp = pNext;
...
void add_to_book_list(struct ListBook* lst, struct Book* p)
{
...
struct ListBook* read_books(int fl)
{
    FILE* file;
    if (fl == 1) file = fopen("Books.txt", "r");
    else file = fopen("Booking.txt", "r");
    if (!file) exit(2);
   struct ListBook* temp = (struct ListBook*)malloc(sizeof(struct ListBook));
...
void load_book_to_file(struct Book* book, int fl, int count) 
{
    FILE* file;
    if (fl == 1) file = fopen("Books.txt", "a");
    else file = fopen("Booking.txt", "a");
    if (!file) exit(2);
...
void books_file_rewrite(struct ListBook* list)
{
...
void print_books(struct ListBook* list)
{
   struct  Book* temp = list->head;
    while (temp) {
